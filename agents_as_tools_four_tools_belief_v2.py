...
# -*- coding: utf-8 -*-
"""
Four-tool agentic GRPO pipeline with explicit BELIEF_STATE memory.

Tools:
  1) extractor_tool  - pull salient facts, entities, numbers
  2) reasoner_tool   - compare hypotheses
  3) skeptic_tool    - find counter-evidence / uncertainty
  4) retriever_tool  - locate relevant snippets
...
def append_jsonl_locked(path: str, rows: List[Dict[str, Any]]) -> None:
    """FIX: fcntl-locked append for multi-rank safety."""
    if not rows:
        return
    ensure_dir(os.path.dirname(path) or ".")
...
# =========================================================
EXTRACTOR_SYS = (
    "You are an extractor tool.\n"
    "Only extract salient information. Do not perform final reasoning.\n"
    "Return ONLY a JSON object with keys:\n"
    "  key_facts: list[str]\n"
    "  entities: list[str]\n"
    "  numbers_or_measurements: list[str]\n"
    "  confidence: float 0.0~1.0\n"
)

REASONER_SYS = (
    "You are a reasoner tool.\n"
    "Perform chain-style analysis and compare hypotheses, but do not output a final answer label.\n"
    "Return ONLY a JSON object with keys:\n"
    "  hypotheses: list[{name:str, support:str, weakness:str}]\n"
    "  comparison: list[str]\n"
...
SKEPTIC_SYS = (
    "You are a skeptic / critic tool.\n"
    "Find counter-evidence, uncertainty, missing assumptions, plausible failure modes.\n"
    "Return ONLY a JSON object with keys:\n"
...
RETRIEVER_SYS = (
    "You are a retriever-style tool.\n"
    "Locate the most relevant snippets or candidate knowledge pieces from the provided context.\n"
    "Return ONLY a JSON object with keys:\n"
    "  retrieved_snippets: list[{sid:int, text:str, score:float}]\n"
    "  coverage_note: str\n"
    "  confidence: float 0.0~1.0\n"
)

TOOL_SPECS = {
    "extractor_tool": EXTRACTOR_SYS,
    "reasoner_tool": REASONER_SYS,
    "skeptic_tool": SKEPTIC_SYS,
    "retriever_tool": RETRIEVER_SYS,
}
TOOL_ORDER = ["extractor_tool", "reasoner_tool", "skeptic_tool", "retriever_tool"]
...
# =========================================================
def _normalize_extractor(obj: Dict[str, Any]) -> Dict[str, Any]:
    for k in ["key_facts", "entities", "numbers_or_measurements"]:
        v = obj.get(k, [])
        obj[k] = [str(x)[:160] for x in v[:8]] if isinstance(v, list) else []
    try:
...
def _normalize_reasoner(obj: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for it in hyps[:5]:
            if isinstance(it, dict):
                norm_h.append({
                    ...
                    "support": str(it.get("support", ""))[:180],
                    "weakness": str(it.get("weakness", ""))[:180],
                })
    obj["hypotheses"] = norm_h
    cmp_ = obj.get("comparison", [])
    obj["comparison"] = [str(x)[:180] for x in cmp_[:5]] if isinstance(cmp_, list) else []
    obj["best_supported_hypothesis"] = str(obj.get("best_supported_hypothesis", ""))[:160]
    uf = obj.get("uncertainty_flags", [])
    obj["uncertainty_flags"] = [str(x)[:120] for x in uf[:5]] if isinstance(uf, list) else []
    try:
        obj["confidence"] = float(obj.get("confidence", 0.6))
    except Exception:
        obj["confidence"] = 0.6
...
def _normalize_skeptic(obj: Dict[str, Any]) -> Dict[str, Any]:
    for k in ["counter_evidence", "ambiguities", "missing_information", "risk_flags"]:
        v = obj.get(k, [])
        obj[k] = [str(x)[:180] for x in v[:6]] if isinstance(v, list) else []
    try:
        obj["confidence"] = float(obj.get("confidence", 0.6))
    except Exception:
        obj["confidence"] = 0.6
...
def _normalize_retriever(obj: Dict[str, Any]) -> Dict[str, Any]:
    rs = obj.get("retrieved_snippets", [])
    norm_rs = []
    if isinstance(rs, list):
        for it in rs[:6]:
            if isinstance(it, dict):
                ...
                norm_rs.append({
                    "sid": sid,
                    "text": str(it.get("text", ""))[:240],
                    "score": max(0.0, min(1.0, sc)),
                })
    obj["retrieved_snippets"] = norm_rs
    obj["coverage_note"] = str(obj.get("coverage_note", ""))[:200]
    try:
        obj["confidence"] = float(obj.get("confidence", 0.6))
    except Exception:
        obj["confidence"] = 0.6
...
def _weak_tool_target(tool_name: str, q: str, ctx: str, candidates: List[Dict[str, Any]], evidence: List[Dict[str, Any]]) -> Dict[str, Any]:
    if tool_name == "extractor_tool":
        return _normalize_extractor({
            "key_facts": [str(e["text"])[:160] for e in evidence[:5]],
            "entities": tokenize_words(q)[:8],
            "numbers_or_measurements": re.findall(r"\b\d+(?:\.\d+)?\b", q + " " + ctx)[:8],
            "confidence": 0.6,
        })
    if tool_name == "reasoner_tool":
        hyps = []
        for i in range(min(3, len(evidence))):
            hyps.append({
                "name": f"Hypothesis {i+1}",
                "support": evidence[i]["text"][:160],
                "weakness": "Evidence may be incomplete or indirect.",
            })
        return _normalize_reasoner({
            "hypotheses": hyps,
            "comparison": [
                "Compare how well each hypothesis explains the main symptoms or requirements.",
                "Prefer hypotheses supported by the most direct evidence.",
            ],
            "best_supported_hypothesis": hyps[0]["name"] if hyps else "",
            "uncertainty_flags": WEAK_UNCERTAINTY_FLAGS[:],
            "confidence": 0.6,
        })
    if tool_name == "skeptic_tool":
        return _normalize_skeptic({
            "counter_evidence": ["The available evidence may be indirect or incomplete."],
            "ambiguities": ["Several answer options may partially fit the question."],
            "missing_information": ["Additional discriminative evidence would improve confidence."],
            "risk_flags": WEAK_UNCERTAINTY_FLAGS[:],
            "confidence": 0.6,
        })
    if tool_name == "retriever_tool":
        return _normalize_retriever({
            "retrieved_snippets": [
                {"sid": int(c["sid"]), "text": str(c["text"])[:240], "score": float(max(0.0, min(1.0, c["score"] + 0.2)))}
                for c in candidates[:6]
            ],
            "coverage_note": "Top snippets ranked by overlap with the question.",
            "confidence": 0.6,
        })
    ...
    def add_one(eid: int, variants: int, is_dev: bool) -> None:
        ...
        for _ in range(variants):
            ...
            for tool_name in TOOL_ORDER:
                ...
                obj = _weak_tool_target(tool_name, q, ctx, candidates, evidence)
                if tool_name == "retriever_tool":
...
PLAIN_TOOL_NAME_RE = re.compile(
    r"^\s*(extractor_tool|reasoner_tool|skeptic_tool|retriever_tool)\s*[\(\{]",
...
def init_tool_agents(tool_base_model: str, adapter_paths: Dict[str, str], device: str):
    ...
    if can_share and _shared_tool_base is None:
        try:
            ...
            _tool_agents = {
                "extractor_tool": SharedToolView(_shared_tool_base, "extractor_tool", max_new_tokens=420),
                "reasoner_tool": SharedToolView(_shared_tool_base, "reasoner_tool", max_new_tokens=520),
                "skeptic_tool": SharedToolView(_shared_tool_base, "skeptic_tool", max_new_tokens=420),
                "retriever_tool": SharedToolView(_shared_tool_base, "retriever_tool", max_new_tokens=420),
            }
            if is_main_process():
    ...
    _tool_agents = {}
    for tool_name in TOOL_ORDER:
        max_new = 520 if tool_name == "reasoner_tool" else 420
...
def _tool_input_messages(tool_name: str, eid: int, q: str, ctx: str, candidates: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    sys_prompt = TOOL_SPECS[tool_name]
    if tool_name == "retriever_tool":
        user = f"Example ID: {eid}\nQUESTION:\n{q}\n\nCONTEXT:\n{ctx}\n"
    else:
...
def _fallback_tool_output(tool_name: str, q: str, ctx: str, candidates: List[Dict[str, Any]], evidence: List[Dict[str, Any]]) -> Dict[str, Any]:
    obj = _weak_tool_target(tool_name, q, ctx, candidates, evidence)
    obj["confidence"] = 0.0
    if tool_name == "reasoner_tool":
        flags = obj.get("uncertainty_flags", [])
        if "invalid_tool_output" not in flags:
            flags.append("invalid_tool_output")
        obj["uncertainty_flags"] = flags[:5]
    elif tool_name == "skeptic_tool":
        rf = obj.get("risk_flags", [])
        if "invalid_tool_output" not in rf:
            rf.append("invalid_tool_output")
        obj["risk_flags"] = rf[:6]
    return obj
...
def _normalize_tool_output(tool_name: str, obj: Dict[str, Any]) -> Dict[str, Any]:
    if tool_name == "extractor_tool":
        return _normalize_extractor(obj)
    if tool_name == "reasoner_tool":
        return _normalize_reasoner(obj)
    if tool_name == "skeptic_tool":
        return _normalize_skeptic(obj)
    if tool_name == "retriever_tool":
        return _normalize_retriever(obj)
    raise ValueError(f"Unknown tool_name={tool_name}")
...
def _run_tool(tool_name: str, example_id: int) -> str:
    ...
    msgs = _tool_input_messages(tool_name, eid, q, ctx, candidates)
    ...
    if obj is None:
        obj = _fallback_tool_output(tool_name, q, ctx, candidates, evidence)
...
def extractor_tool(example_id: int) -> str:
    """Extract salient facts from the example context.

    Args:
        example_id: dataset example id from the user message.

    Returns:
        JSON string with key_facts, entities, numbers_or_measurements, confidence.
    """
    return _run_tool("extractor_tool", example_id)


def reasoner_tool(example_id: int) -> str:
    """Compare hypotheses for the example.

    Args:
        example_id: dataset example id from the user message.

    Returns:
        JSON string with hypotheses, comparison, best_supported_hypothesis, confidence.
    """
    return _run_tool("reasoner_tool", example_id)


def skeptic_tool(example_id: int) -> str:
    """Find counter-evidence and uncertainty.

    Args:
        example_id: dataset example id from the user message.

    Returns:
        JSON string with counter_evidence, ambiguities, missing_information, confidence.
    """
    return _run_tool("skeptic_tool", example_id)


def retriever_tool(example_id: int) -> str:
    ...
    Returns:
        JSON string with retrieved_snippets, coverage_note, confidence.
    """
    return _run_tool("retriever_tool", example_id)
...
def build_manager_system_prompt() -> str:
    ...
    return (
        task_line + "\n\n"
        "Tools available (use the model's native tool-calling interface only):\n"
        "  extractor_tool, reasoner_tool, skeptic_tool, retriever_tool\n"
        f"You may call up to {MAX_MANAGER_TOOL_CALLS} tools total.\n\n"
        ...
        "}\n\n"
        "Tool guidance:\n"
        "- extractor_tool: salient facts only.\n"
        "- reasoner_tool: compare hypotheses.\n"
        "- skeptic_tool: counter-evidence and uncertainty.\n"
        "- retriever_tool: relevant snippets.\n"
        "- Prefer tool diversity. Do not call the same tool repeatedly.\n"
        "- Update BELIEF_STATE after each tool result.\n"
...
def build_initial_belief_state(eid: int, q: str, ctx: str) -> Dict[str, Any]:
    ...
    return {
        "problem_type_guess": "multiple_choice" if len(ANSWER_LABELS) >= 4 else "clinical_qa",
        "known_facts": [f"example_id={eid}", f"question_words={len(q_words)}", f"context_words={len(ctx_words)}"],
        "current_hypotheses": [],
        "uncertainties": ["Need to determine which evidence or reasoning path is most diagnostic."],
        "tools_called": [],
        "remaining_budget": MAX_MANAGER_TOOL_CALLS,
        "recommended_next_tool": "extractor_tool",
        "why_this_tool": "Start by extracting salient facts before deeper reasoning.",
    }
...
def _belief_quality_bonus(belief_obj: Optional[Dict[str, Any]]) -> float:
    """FIX: cap belief bonus so it can't dominate correctness signal.

    Max 0.10 total across all sub-bonuses."""
    if belief_obj is None:
        return 0.0
    bonus = 0.04  # baseline for having any belief block
...
def routing_aware_reward(prompts=None, completions=None, ground_truth=None, example_id=None, **kwargs):
    """Shaped reward. Correctness dominates; belief+tool signals give dense learning signal.

    Components (max ~1.40, min ~-1.0):
      +0.10  valid format (parsed ANSWER_*)
      +1.00  correct label          <-- dominant
      +0.05  at least one tool call
      +diversity bonus (0 ~ 0.15)
      +belief quality bonus (0 ~ 0.10)   <-- capped
      +recommended-tool alignment (0 or 0.05)
      -budget penalty
      -repeated-tool penalty
      -0.25 last turn mixes tool artifacts with final answer
...
def analyze_routing_trace(raw_trace_jsonl: str, out_json: str = "", out_txt: str = "") -> Dict[str, Any]:
    ...
    if not rows:
        ...
        for r in rows:
            ...
            by_route[route].append(float(r.get("reward", 0.0)))
            tool_counter.update(r.get("tool_call_names", []))
            pred = r.get("pred")
            gt = r.get("ground_truth")
            by_route_correct[route].append(int(pred == gt and pred is not None))
            belief_present += int(bool(r.get("belief_state_present")))

        summary = {
            "num_rows": len(rows),
            "belief_state_present_rate": belief_present / max(1, len(rows)),
            ...
            "route_avg_reward": {k: sum(v) / max(1, len(v)) for k, v in by_route.items()},
        }
    ...
    if out_txt:
        ...
        lines = [
            ...
            f"belief_state_present_rate: {summary.get('belief_state_present_rate', 0.0):.4f}",
            "route_counts:",
        ]
...
def train_manager_grpo_from_splits(
    ...
    adapter_paths = {
        "extractor_tool": extractor_adapter,
        "reasoner_tool": reasoner_adapter,
        "skeptic_tool": skeptic_adapter,
        "retriever_tool": retriever_adapter,
    }
    ...
    if use_wandb:
        ...
        run_name = wandb_run_name.strip() or f"grpo4_{os.path.basename(save_dir.rstrip('/\\'))}_{int(time.time())}"
    ...
    trainer_kwargs = {
        ...
        "rollout_func": None,
        "tools": [extractor_tool, reasoner_tool, skeptic_tool, retriever_tool],
    }
...
def main():
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "train_extractor_tool", "train_reasoner_tool",
            "train_skeptic_tool", "train_retriever_tool",
    ...
    parser.add_argument("--extractor_tool_out", type=str, default="extractor_tool_adapter")
    parser.add_argument("--reasoner_tool_out", type=str, default="reasoner_tool_adapter")
    parser.add_argument("--skeptic_tool_out", type=str, default="skeptic_tool_adapter")
    parser.add_argument("--retriever_tool_out", type=str, default="retriever_tool_adapter")
    ...
    tool_stage_map = {
        "train_extractor_tool": ("extractor_tool", args.extractor_tool_out),
        "train_reasoner_tool": ("reasoner_tool", args.reasoner_tool_out),
        "train_skeptic_tool": ("skeptic_tool", args.skeptic_tool_out),
        "train_retriever_tool": ("retriever_tool", args.retriever_tool_out),
    }
    ...
    if args.stage == "train_manager_grpo":
        ...
        train_manager_grpo_from_splits(
            ...
            extractor_adapter=args.extractor_tool_out,
            reasoner_adapter=args.reasoner_tool_out,
            skeptic_adapter=args.skeptic_tool_out,
            retriever_adapter=args.retriever_tool_out,
            seed=args.seed,
